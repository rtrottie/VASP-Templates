...
def run_vasp(override=[], suffix=''):
    ...
    handlers = [VaspErrorHandler('vasp.log'), PositiveEnergyErrorHandler(), NonConvergingErrorHandler_toDamped(nionic_steps=20, change_algo=True)]
    if 'PBS_START_TIME' in os.environ:
        ...
        if buffer_time*2.5 > walltime:
            raise Exception('Not Enough Time')
        handlers += [WalltimeHandler(wall_time=remaining_time, buffer_time=buffer_time, electronic_step_stop=True,)]
